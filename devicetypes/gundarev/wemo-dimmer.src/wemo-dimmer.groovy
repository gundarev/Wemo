 ...
 */
 ...
 // UI tile definitions
    tiles(scale: 2) {
        multiAttributeTile(name:"rich-control", type: "light", canChangeIcon: true){
    ...
    if (bodyString) {
        ...
        def body = new XmlSlurper().parseText(bodyString)
 		...
 		if (body?.property?.TimeSyncRequest?.text()) {
        	log.trace "1Got TimeSyncRequest"
        	result << timeSyncResponse()
        } else if (body?.Body?.SetBinaryStateResponse?.BinaryState?.text()) {
 			log.trace "2Got SetBinaryStateResponse = ${body?.Body?.SetBinaryStateResponse?.BinaryState?.text()}"
            ...
            result << createEvent(name: "switch", value: value, descriptionText: "Switch is ${value}", displayed: dispaux)
 			...
 			log.debug "5Notify: TimeZoneNotification = ${body?.property?.TimeZoneNotification?.text()}"
 		} else if (body?.Body?.GetBinaryStateResponse?.BinaryState?.text()) {
        log.trace body?.Body
 			def value = body?.Body?.GetBinaryStateResponse?.BinaryState?.text().substring(0, 1).toInteger() == 0 ? "off" : "on"
 			def brightness = body?.Body?.GetBinaryStateResponse?.brightness?.text().toInteger()
 			log.trace "GetBinaryResponse: BinaryState = ${value}, ${body.property.BinaryState}, brightness= ${brightness}"
            ...
            def dispaux3 = device.currentValue("level") != value
            ...
            result << createEvent(name: "level", value: brightness, descriptionText: "level is ${vbrightnessalue}", displayed: dispaux3)
 		}
 	}
 result
}
...
def refresh() {
 	log.debug "Executing WeMo Dimmer 'subscribe', then 'timeSyncResponse', then 'poll'"
